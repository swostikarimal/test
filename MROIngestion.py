...
filterdate = (pd.to_datetime(today) - pd.Timedelta(days=150)).strftime("%Y-%m-%d")
...
def getTables(updates, status):
    ...
    for table in tables:
        ...
        if tableinfo.HaveDate == "YES":
            try:
                created = updates.filter(F.col("TableName") == tablename).select("upto").collect()[0][0]
                created = (pd.to_datetime(created) - pd.Timedelta(days=150)).strftime("%Y-%m-%d")

                t1query = f'''
                    SELECT * FROM [{tablename}]
                    WHERE CAST(CreationTime AS DATE) >= '{created}'
                '''
                sdf = featchData(t1query, jdbc_url, connection_properties)
                spec["datesince"] = created

            except Exception as e:
                print("Table Does not exist in the update table")
                t1query = f'''
                    SELECT * FROM [{tablename}]
                '''
                sdf = featchData(t1query, jdbc_url, connection_properties)
...
filterdate = (pd.to_datetime(today) - pd.Timedelta(days=200)).strftime("%Y-%m-%d")

# COMMAND ----------

spark.table("analyticadebuddha.mro.mroupdates").display()
