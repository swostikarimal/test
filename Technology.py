...
'''
The data ingestion for the Tech detals that contains the and their engagement with the website required by the business
@PARAMS
...
pkcolmns = ["date", "deviceCategory", "screenResolution", "operatingSystem"] 
...
def fetch_ga_data(client, property_id: str) -> pd.DataFrame:
    ...
    request = RunReportRequest(
        ...
        dimensions=[
            Dimension(name="date"), Dimension(name="deviceCategory"), Dimension(name="screenResolution"), Dimension(name="operatingSystem")
        ],
        metrics=[
...
def process_user_tech(client, property_id: str, subfolder: str):
    """
...
def main():
    ...
    properties = {
        "properties/347471683": {"subfolder": "reporting/acquisition/audienceandtech", "overwrite": True}
    }
